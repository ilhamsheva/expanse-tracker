...
export const getDashboardData = async (req, res) => {
  ...
  try {
    ...
    const totalIncome30Days = last30DaysIncomeTransaction.reduce((sum, total) => {
        sum + total.amount;
    });
    ...
    const totalExpense30Days = last30DaysExpenseTransaction.reduce((sum, total) => {
        sum + total.amount;
    });

    // Fetch 7 last data income and expanse
    const lastTransactions = [
        ...[(await Income.find({userId}).sort({date: -1}).limit(7)).map((item) => ({
            ...item.toObject(),
            type: "income",
        }))],

        ...[(await Expense.find({userId}).sort({date: -1}).limit(7)).map((item) => ({
            ...item.toObject(),
            type: "expense",
        }))],
